...
| 1 Graph core | handles, builder with declare-and-bind (binding state in `RHICommandContext`, shader variants), validation, cull, barrier planning, load/store inference (no merging yet: every pass its own physical pass), transient pool, imported/persistent resources, External/Copy passes, JSON dump; shared post chain first, then one change per renderer in the order CPU → GPU → Forward → Deferred, each fully on the graph when merged (no runtime switch); the old pass/render-target APIs and renderer-level `Transition` calls are deleted with the last port | sync validation clean; per-renderer golden graph-shape test (pass list, barrier count, load/store per attachment) from the dump |
...
* **D11. Graph scope.** TLAS build, IBL cook dispatches, readback and clears become graph passes (§8). Uploads stay in a frame prologue with one barrier, because they are issued from many places. BLAS build and Metal compaction stay outside the frame (CPU readback).
...
* **D22. Port renderer by renderer.** Phase 1 lands the graph core with the shared post chain, then one change per renderer (CPU → GPU → Forward → Deferred), each fully on the graph when merged. There is no runtime graph/legacy switch; old pass APIs are deleted with the last port.
