...
import nrd_common
from nrd_common import NUM_FRAMES, PROJECT_ROOT, ft, run_sweep
...
def parse_args():
    ...
    p.add_argument("--framework", default="macos", choices=ft.SUPPORTED_FRAMEWORKS)
...
def collect(framework, prefix, work_dir):
    import shutil
    shot_dir = ft.get_screenshot_dir(framework)
    paths = []
...
def main():
    global SETTLE
    ft.install_dependencies()
    args, passthrough = parse_args()
    SETTLE = args.settle

    work_dir = os.path.join(ft.get_screenshot_dir(args.framework), "static_stability")
