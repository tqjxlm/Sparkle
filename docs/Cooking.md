...
CI cooks once per pipeline run on a macos-release node (a real Metal GPU cooks in seconds, unlike software Vulkan) and injects the result into every product's release package; the test stage runs the released windows package under software Vulkan and the released macos package on a Metal GPU, each with a log gate that fails any run cooking on the fly. A single shared cook serves all platforms because the image payload layout is a cross-backend contract (see Artifacts); genuinely platform-specific content would add per-platform cook nodes. The stage graph, injection and re-signing rules live in [CI.md](CI.md).
