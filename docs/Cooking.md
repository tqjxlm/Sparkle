...
One macos-release node cooks once per pipeline run (a real Metal GPU cooks in seconds, unlike software Vulkan) and publishes the result; every product's release package gets that output injected and is re-signed where injection breaks the signature; the test stage runs the released windows package with a log gate that fails any run cooking on the fly. A single shared cook serves all platforms because the image payload layout is a cross-backend contract (see Artifacts); genuinely platform-specific content would add per-platform cook nodes. The stage graph and its dependency rules live in [CI.md](CI.md).
