...
* `windows-glfw-release` and `ubuntu-android-release` render on the **same driver** — Mesa lavapipe (`llvmpipe`), the android one inside an x86_64 emulator — so a case whose subject is GPU or driver behaviour learns nothing from the second copy. What is unique to them is the host runtime: the MSVC-built binary and Win32 file/path semantics, and the Android app lifecycle, asset plumbing and surface handling. `windows-glfw-release` carries the shared software-rendering and x86 load.
* `ubuntu-glfw-release` is the exception: the linux product is arm64 and its cell is a self-hosted Jetson (see [CI.md](CI.md)), whose Tegra GPU is the only one in the matrix exposing `VK_KHR_ray_query`. That makes it the only cell that renders on hardware ray tracing and the only home for `gpu_render_static`. What it no longer proves is anything about the software rasterizer, which is why the Windows cell keeps that job.
* `macos-macos-release` (Metal) and `macos-glfw-release` (Vulkan through MoltenVK) run on a physical GPU, as does `ubuntu-glfw-release`; those two plus `macos-ios-release` and `ubuntu-glfw-release` are the ARM columns. That leaves `windows-glfw-release` as the **only x86 column with a desktop runtime**, so anything arch-sensitive has to include it: astcenc is built NEON on ARM and SSE4.1 elsewhere ([thirdparty/CMakeLists.txt](../thirdparty/CMakeLists.txt)), which is why `texture_compression` and `sky_compression` carry a windows mark — it is the whole SSE4.1 side of that split now that the linux product is arm64.
