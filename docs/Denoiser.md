...
`--nrd_radiance_fp16` (default on) uses RGBA16F shared radiance inputs, half the input bandwidth. Turn it off (fp32) to rule out radiance/hitT quantization when debugging either provider.

### Display handoff

Neither provider denoises forever: a resolve pass cross-fades the provider output into the progressive accumulator over the same sample window, and the display equals the accumulator once the window closes. `max_spp` below the window opts out entirely, so the max_spp=1 motion harnesses keep filming denoised output. `DenoiserHandoff` owns the schedule — window bounds, the ramp, the final-frame pin and the opt-out — because the gates compare providers against each other and a per-provider copy would let them drift. Each provider chapter documents its own resolve mechanics.

Every provider's per-stage GPU timing goes through `PassTimingAggregator`, which harvests each frame slot's pending times before recording which stages the frame dispatches into it, and logs running and final per-stage means under the provider's own tag.

### Tests

[tests/denoiser/run_denoiser_gates.py](../tests/denoiser/run_denoiser_gates.py) is the one-command suite, run locally before pushing (the gates that matter here are Metal-side, and no CI runner offers a Metal device with ray tracing — the hosted macos runners have a physical Apple Silicon GPU, but their paravirtualized device reports `supportsRaytracing == false`. The self-hosted Jetson cell does have hardware ray tracing, which is what lets `gpu_render_static` run in CI, but it reaches NRD through Vulkan and never touches the Metal backend these gates are about): converged flicker, firefly, motion noise (yaw + pitch arms). The suite and its gate scripts take `--denoiser nrd|metalfx` (default `nrd`) so every gate runs against any backend. [tests/denoiser/denoiser_compare.py](../tests/denoiser/denoiser_compare.py) renders a side-by-side backend table — static noise/flicker/FLIP vs a locally converged reference, yaw/pitch motion noise, and per-frame cost — with `--render_scale` for the reconstruction regime. The probe run requires the app's `effective pipeline: Gpu` log marker — on GPUs without hardware ray tracing the app silently falls back to forward rendering and still exits 0, so exit codes alone prove nothing. Motion gates assert all captures are pairwise distinct (a frozen/black capture sequence otherwise passes statistical thresholds). `--test_case nrd_probe` checks that every ReBLUR pipeline compiles to a PSO through the production MetalNrdBackend path. `--test_case denoiser_handoff` covers the shared handoff schedule as pure arithmetic, so it runs on every triplet in the suite rather than only where a denoiser can execute.
...
### Display pipeline: scaler handoff and freeze
