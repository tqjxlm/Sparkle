...
Neither provider denoises forever: a resolve pass cross-fades the provider output into the progressive accumulator over the same sample window, and the display equals the accumulator once the window closes. `max_spp` below the window opts out entirely, so the max_spp=1 motion harnesses keep filming denoised output. Each provider chapter documents its own resolve mechanics.
