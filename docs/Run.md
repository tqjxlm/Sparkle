...
| `denoiser`          | string | `off`      | gpu         | Path-tracing denoiser: `off`, `auto`, `nrd`, or `metalfx`. `auto` prefers MetalFX and falls back to NRD                                                              |
| `nrd`               | bool   | false      | gpu         | Startup compatibility alias: `true` selects `denoiser=nrd`; use `denoiser` for new configurations and runtime changes                                                |
